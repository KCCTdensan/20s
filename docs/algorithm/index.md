...
```
数が小さい順に並んでいる配列において，答えの下限である右端と上限である左端を決めた時に，xはその配列の右端と左端の真ん中の値以上かどうか
```
とは言ってもこれを見ただけでは何のことかさっぱりわからないと思うので，例を挙げながら解説していきます．

入力例1の1つ目のクエリ(x=3)を例としてあげると次のようになります．


![](img/binary0.png)

始めは，答えの下限は配列の左外(-1の場所)に，答えの上限は配列の右外(Nの場所)に設定しておきます．

今回の一行目のクエリではx=3であり，この時真ん中の値である5はx以上なので，答えは少なくとも5よりも右側には存在しないことが分かります．
...
今回の真ん中の値である1はx以上ではないことから，答えは1から左には存在しないことが分かるので，左端を真ん中まで持っていきます．
